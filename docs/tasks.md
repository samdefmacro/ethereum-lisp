...
- [x] `DEVNET-RUNNER-TXPOOL-LIFETIME`: Make the geth/Hive
  ...
  - Dependencies: `DEVNET-RUNNER-TXPOOL-SLOT-LIMITS`,
    `DEVNET-RUNNER-TXPOOL-QUEUE-LIMITS`,
    `DEVNET-RUNNER-TXPOOL-LOCAL-EXEMPTIONS`
  - Acceptance: `--txpool.lifetime DURATION` parses as a non-negative
    duration, is reported in devnet summaries/readiness/telemetry, imports
    from geth TOML `[Eth.TxPool] Lifetime` when present, and causes stale
    queued-view public txpool entries to disappear from `txpool_status`,
    `txpool_content`, `txpool_contentFrom`, `txpool_inspect`, pending-tag
    views, and hash lookup after deterministic cleanup. Pending executable
    transactions must not be expired by this slice, same-sender replacements
    must refresh the effective age, and tests must not rely on wall-clock
    sleeps.
  - Validation: focused txpool/CLI coverage while iterating;
...
- [x] `PINNED-V5.4.0-BLOCKCHAIN-WIDEN`: Use the bounded official v5.4.0
  ...
  - Result (2026-06-27): added 13 more official Shanghai
    `engineNewPayloadV2` replay selectors covering EIP-2930 access-list
    boundaries plus PUSH0 and warm-coinbase Engine payloads, including the
    DELEGATECALL variants. The DELEGATECALL drift was caused by the Engine
    fixture-derived Shanghai chain config not activating Homestead/EIP150/
    EIP155/EIP158; those early forks are now enabled for the replay harness.
...
- [x] `PINNED-V5.4.0-BLOCKCHAIN-PUSH-WIDEN`: Continue official v5.4.0 Engine
  ...
  - Result (2026-06-27): added all 32 official
    ...
    fixture cases.
- [x] `PINNED-V5.4.0-BLOCKCHAIN-DUP-WIDEN`: Continue official v5.4.0 Engine
  replay widening with selector-probed Frontier DUP opcode cases materialized
  under the Shanghai fork.
  - Result (2026-06-28): added all 16 official
    `frontier/opcodes/test_dup.json` selectors for DUP1 through DUP16 as
    Shanghai `engineNewPayloadV2` replay cases. A targeted probe replayed all
    16 selectors through the Engine import path and verified post-state. The
    pinned smoke gate now covers 94 official state selectors, 53 official
...
- [x] `DEVNET-RUNNER-TXPOOL-QUEUE-LIMITS`: Make the geth/Hive account and
  global txpool queue flags affect nonce-gap public raw-transaction admission.
  - Result (2026-07-04): `--txpool.accountqueue` and
    `--txpool.globalqueue` now parse as non-negative integer caps, are reported
    in devnet JSON summaries and process telemetry, and reject new queued
    `eth_sendRawTransaction` submissions when the sender-local or global
    queued subpool is already at capacity. Same-sender/same-nonce replacements
    continue to use the replacement price-bump policy and do not consume an
    additional queue slot. Geth TOML `[Eth.TxPool] AccountQueue` and
    `[Eth.TxPool] GlobalQueue` import through the same path, and explicit CLI
...
- [x] `DEVNET-RUNNER-KZG-CAPABILITY-OPT-IN`: Prove explicit runner KZG
  ...
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live oversized `engine_getPayloadBodiesByRangeV2` request with
    `count = 0x401`, proving the existing `-38004` / "must not exceed 1024"
    contract at the runner boundary instead of only through in-process tests.
    The smoke assertions now fail clearly if the live request returns success,
    omits the error, changes the documented code/message, or includes a
    spurious success `result`; the nested report records
    `preparedPayloadBodiesByRangeV2OversizedErrorCode` and
    ...
    requests. Focused escalated smoke, direct escalated
    `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage, `git diff
    --check`, and independent verifier review passed. Residual risk is now
    limited to the non-positive `start` / `count` validation edge and any
    broader malformed-request envelope drift at the same runner boundary.
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    live zero-start and zero-count `engine_getPayloadBodiesByRangeV2`
    requests, proving the existing `-32602` / "start and count must be
    positive numbers" contract at the runner boundary instead of only through
    in-process validation. The smoke assertions now fail clearly if either
    live request returns success, omits the error, changes the documented
    code/message, or includes a spurious success `result`; the nested report
    records `preparedPayloadBodiesByRangeV2ZeroStartErrorCode`,
    `preparedPayloadBodiesByRangeV2ZeroStartErrorMessage`,
    `preparedPayloadBodiesByRangeV2ZeroCountErrorCode`, and
  ...
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live malformed-count `engine_getPayloadBodiesByRangeV2` request with
    `count = "0xzz"` and proves the existing invalid-params `-32602` /
    "count must be a non-negative quantity" envelope through the real
    listener path instead of only through in-process validation. The smoke
    assertions now fail clearly if the live request returns success, omits the
    error, changes the documented code/message, or includes a spurious
    success `result`; the nested report records
    `preparedPayloadBodiesByRangeV2MalformedCountErrorCode` and
    ...
    `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and independent verifier review passed. Residual risk
    is now limited to broader params-envelope drift at the same runner
    boundary.
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live one-element `engine_getPayloadBodiesByRangeV2` params array and
    proves the existing invalid-params `-32602` /
    "engine_getPayloadBodiesByRangeV2 param count is missing" envelope through
    the real listener path instead of only through in-process validation. The
    smoke assertions now fail clearly if the live request returns success,
    omits the error, changes the documented code/message, or includes a
    spurious success `result`; the nested report records
    `preparedPayloadBodiesByRangeV2ParamsEnvelopeErrorCode` and
    ...
    `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and independent verifier review passed. Residual risk
    is now limited to non-array `params` invalid-request drift at the same
    runner boundary.
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live scalar non-array `engine_getPayloadBodiesByRangeV2` `params`
    request and proves the existing JSON-RPC invalid-request
    `-32600` / "Invalid Request" envelope through the real listener path
    instead of only through in-process validation. The smoke assertions now
    fail clearly if the live request returns success, omits the error, changes
    the documented code/message, or includes a spurious success `result`; the
    nested report records
  ...
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    ...
    runner boundary.
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live non-empty object-valued `engine_getPayloadBodiesByRangeV2`
    `params` request such as `{"start":"0x1","count":"0x1"}` and proves the
    existing invalid-params `-32602` / "start must be a non-negative
    quantity" envelope through the real listener path instead of only through
    in-process validation. The smoke assertions now fail clearly if the live
    request returns success, omits the error, changes the documented
    code/message, or includes a spurious success `result`; the nested report
    records `preparedPayloadBodiesByRangeV2ObjectParamsErrorCode` and
    ...
    Engine requests, including the child `--max-connections` budget and
    shutdown telemetry checks. Focused escalated smoke, direct escalated
    `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and independent verifier review passed. Residual risk
    is now limited to empty-object or broader malformed-object `params`
    drift at the same runner boundary.
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live empty-object `engine_getPayloadBodiesByRangeV2` `params` request
    such as `{}` and proves the existing invalid-params `-32602` /
    "engine_getPayloadBodiesByRangeV2 params must include start and count"
    envelope through the real listener path instead of only through
    in-process validation. The smoke assertions now fail clearly if the live
    request returns success, omits the error, changes the documented
    code/message, or includes a spurious success `result`; the nested report
    records `preparedPayloadBodiesByRangeV2EmptyObjectParamsErrorCode` and
    ...
    twenty-three Engine requests, including the child `--max-connections`
    budget and shutdown telemetry checks. Focused escalated smoke, direct
    escalated `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and independent verifier review passed. Residual risk
    is now limited to single-key or broader malformed-object `params` drift
    at the same runner boundary.
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live single-key object-valued `engine_getPayloadBodiesByRangeV2`
    `params` request such as `{"count":"0x1"}` and proves the existing
    invalid-params `-32602` / "start must be a non-negative quantity"
    envelope through the real listener path instead of only through
    in-process validation. The smoke assertions now fail clearly if the live
    request returns success, omits the error, changes the documented
    code/message, or includes a spurious success `result`; the nested report
    records `preparedPayloadBodiesByRangeV2MissingStartObjectParamsErrorCode`
    and `preparedPayloadBodiesByRangeV2MissingStartObjectParamsErrorMessage`;
    and the nested KZG connection/shutdown contract expands from
    twenty-three to twenty-four Engine requests, including the child
    `--max-connections` budget and shutdown telemetry checks. Focused
    escalated smoke, direct escalated
    `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and verifier review passed. Residual risk is now
    limited to missing-count or broader malformed-object `params` drift at
    the same runner boundary.
- [x] `DEVNET-RUNNER-KZG-ERROR-TELEMETRY`: Lock invalid KZG verifier
  - Result (2026-07-06): the same engine-only `kzgOptIn` child now also sends
    a live single-key object-valued `engine_getPayloadBodiesByRangeV2`
    `params` request such as `{"start":"0x1"}` and proves the current live
    and in-process invalid-params `-32602` / "start must be a non-negative
    quantity" envelope through the real listener path. This closes the stale
    missing-count assumption from the previous run specification: object-valued
    `params` are still normalized positionally, so the listener reports the
    same start-quantity error rather than a distinct missing-count message.
    The smoke assertions now fail clearly if the live request returns success,
    omits the error, changes the documented code/message, or includes a
    spurious success `result`; the nested report records
    ...
    twenty-five Engine requests, including the child `--max-connections`
    budget and shutdown telemetry checks. Focused escalated smoke, direct
    escalated `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and verifier review passed. Residual risk is now
  ...
  - Result (2026-07-06): the same engine-only non-KZG smoke now also sends one
    live hidden `engine_getPayloadBodiesByHashV2` request and proves the
    listener rejects it with JSON-RPC `-32601` / `"Method not found"` instead
    of returning a success `result`. The engine-only report now records
    `hiddenPayloadBodiesByHashV2Status`,
    `hiddenPayloadBodiesByHashV2ErrorCode`, and
    `hiddenPayloadBodiesByHashV2ErrorMessage`; the engine-only serve
    connection contract expands from eight to nine Engine requests to account
    for the extra live probe; and direct CLI coverage locks the updated
    shutdown telemetry counts alongside the by-range rejection evidence.
    Focused escalated engine-only smoke, focused escalated
    `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and verifier review passed. Residual risk is now
  ...
  - Result (2026-07-06): the same engine-only non-KZG smoke now also sends one
    ...
    Phase B runner work.
  - Result (2026-07-06): the same engine-only non-KZG smoke now also sends one
    live hidden `engine_getBlobsV2` request and proves the listener rejects it
    with JSON-RPC `-32601` / `"Method not found"` instead of returning a
    success `result`. The engine-only report now records
    `hiddenBlobsV2Status`, `hiddenBlobsV2ErrorCode`,
    `hiddenBlobsV2ErrorMessage`, and the explicit negative capability bit
    `engineCapabilityHasGetBlobsV2`; direct core coverage now proves
    `engine_getBlobsV2` is rejected through the same non-KZG method filter;
    and the engine-only serve connection/shutdown contract expands from ten to
    eleven Engine requests to account for the extra live probe. Focused
    escalated engine-only smoke, focused escalated
    `DEVNET-SMOKE-GATE-SCRIPT-ENGINE-ONLY-SERVE-MODE` coverage,
    `git diff --check`, and verifier review passed. Residual risk is now
    limited to the sibling listener-boundary hidden-method proof for live
    non-KZG `engine_getBlobsV3` or a deliberate pivot to broader Phase B
    runner work.
- [x] `DEVNET-RUNNER-KZG-ERROR-TELEMETRY`: Lock invalid KZG verifier
...
- [x] `DEVNET-RUNNER-RESTORED-LOG-FILTERS`: Lock restored public log filter
  ...
  - Result (2026-06-30): the restored-database public RPC smoke now creates
    ...
    reports.
- [x] `DEVNET-RUNNER-RESTORED-BLOCK-FILTERS`: Lock restored public block filter
  lifecycle behavior at the standalone devnet process/database boundary.
  - Result (2026-06-30): the restored-database public RPC smoke now creates an
    `eth_newBlockFilter`, verifies its initial `eth_getFilterChanges` result
    is empty, rejects `eth_getFilterLogs` on that block-filter id, uninstalls
    it, and verifies the removed id returns the expected missing-filter
    `eth_getFilterChanges` error. The smoke report exposes block-filter id,
    change count, wrong-method error code, uninstall result, and removed-filter
    error code, with CLI tests asserting those fields for single-case and
    suite reports.
...
- [x] `PUBLIC-RPC-PENDING-BLOCK-BODY`: Make
  `eth_getBlockByNumber("pending", FULL)` expose the visible pending txpool
...
- [x] `DEVNET-RUNNER-PENDING-BLOCK-BODY`: Lock pending block body visibility at
  ...
  - Result (2026-06-29): the standalone devnet smoke gate's restored-database
    txpool probe now sends `eth_getBlockByNumber("pending", true)` through the
    public listener, verifies the pending block has no committed block hash,
...
- [x] `DEVNET-RUNNER-SCRIPT-TXPOOL`: Lock public txpool admission and visibility
  ...
  - Result (2026-06-29): runner-facing `scripts/ethereum-lisp.lisp`
    ...
    `txpool_contentFrom`, `txpool_inspect`, and a public
    `eth_newPendingTransactionFilter` / `eth_getFilterChanges` round trip that
...
- [x] `DEVNET-RUNNER-STANDALONE-PENDING-FILTER-LIFECYCLE`: Lock pending
  transaction filter drain/uninstall behavior at the standalone devnet smoke
  surface.
  - Result (2026-06-30): the standalone devnet smoke gate now polls the
    pending transaction filter a second time to verify the change queue is
    drained, uninstalls it through `eth_uninstallFilter`, verifies the removed
    id returns the expected `eth_getFilterChanges` missing-filter JSON-RPC
    error, exposes the lifecycle fields in JSON and text output, and updates
    the public txpool connection contract for runner accounting.
...
- [x] `DEVNET-RUNNER-SCRIPT-BLOCK-FILTER`: Lock public block filter change
  ...
  - Result (2026-06-30): runner-facing `scripts/ethereum-lisp.lisp`
    serve-mode subprocess coverage now creates an `eth_newBlockFilter` through
    the public listener before authenticated Engine import, then polls
    `eth_getFilterChanges` after the imported child is accepted and forkchoice
...
- [x] `STORE-CHAIN-INTERFACE`: Define a chain-store interface over the current
  ...
  - Progress:
    ...
    - The live chain-store txpool insertion boundaries now also reject
      subpool/type mismatches, preventing in-memory txpool state from producing
...
- [x] `ENGINE-EXECUTE-NEWPAYLOAD`: Route `engine_newPayload` through block
  ...
  - Progress: added EEST-shaped typed transaction replay coverage for Shanghai
    access-list and dynamic-fee `engineNewPayloads` cases. The in-repo replay
    selector now materializes six source cases, and the Engine replay import
    path validates legacy, EIP-2930, EIP-1559, log-producing call, and
    block-RLP materialization coverage through the same Phase A gate.
  - Progress: added EEST-shaped contract creation replay coverage for Shanghai
    top-level creation and internal CREATE2 `engineNewPayloads` cases. The
    in-repo replay selector now materializes eight source cases, and the
...
- [x] Extract txpool state from the Engine payload memory store.
  ...
  - Progress: hardened txpool snapshot copies so pending, queued, basefee, and
    blob subpools copy transaction objects by canonical encoding while sharing
    the same copied instance between each subpool table and sender/nonce index.
    Rollback snapshots no longer alias caller-mutated transaction instances.
...
- [x] Add a file-backed development database backend.
  ...
  - Result: locked the core Engine forkchoice reinsertion event boundary for
    ...
    displaced valid transaction publishes that transaction hash exactly once
    through `eth_getFilterChanges`.
...
- [x] `DEVNET-RUNNER-NO-COMMAND-ENGINE-ONLY-HANDSHAKE`: Carry the
  Engine-only capability and transition-configuration handshake through the
  direct geth/Hive-shaped `scripts/ethereum-lisp.lisp -- --dev --http=false`
  process boundary.
  - Result (2026-07-03): the no-command Engine-only subprocess test now
    launches with non-default Merge transition override flags, sends
    `engine_exchangeCapabilities`,
    `engine_exchangeTransitionConfigurationV1`, and a mismatched transition
    configuration through the ready-file `/engine` endpoint, asserts the
    Shanghai-safe V1/V2 capability set while blob/KZG-gated methods stay
...
- [x] `DEVNET-RUNNER-NO-COMMAND-DATADIR-SERVE`: Prove initialized datadirs can
  boot through the direct geth/Hive-shaped no-command script process boundary.
  - Result (2026-07-03): subprocess coverage now initializes a temporary
    datadir with geth-shaped pre-command flags, then starts
    `scripts/ethereum-lisp.lisp -- --datadir PATH --http ...` without an
    explicit `devnet` token. The process discovers the stored genesis,
    derived database, and datadir JWT secret, reports split `/engine` and
...
- [x] `PINNED-V5.4.0-BLAKE2F-BLOCKCHAIN-COMPLETE`: Pin the remaining
  official BLAKE2F blockchain replay selectors.
  - Milestone: 3 / Phase A consensus fixtures
  - Dependencies: installed `ethereum/execution-spec-tests` v5.4.0 stable
    fixture root.
  - Acceptance: the unpinned Shanghai `engineNewPayloadV2` selectors from
    `istanbul/eip152_blake2/test_blake2b.json` and
    `istanbul/eip152_blake2/test_blake2b_large_gas_limit.json` are included
    in the pinned Phase A blockchain replay table, classify with zero
    remaining unpinned candidates for the `istanbul/eip152_blake2` prefix, and
    pass the pinned v5.4.0 smoke gate.
  - Validation: `sbcl --script scripts/classify-blockchain-replay-selectors.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --prefix istanbul/eip152_blake2 --json`;
    `sbcl --script scripts/phase-a-drift-map.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --failures-only --json`;
    `sbcl --script scripts/phase-a-smoke-gate.lisp -- --pinned-v5.4.0 --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --json`;
    `sbcl --script tests/run-tests.lisp`.
  - Result (2026-07-01): added 36 official Shanghai
    `engineNewPayloadV2` replay selectors covering BLAKE2F malformed rounds
    lengths, valid round-count/input-offset boundaries, and large gas-limit
    CALL/CALLCODE cases. The pre-pin targeted classifier reported 36/36
    unpinned candidates passing with zero drift/bug/harness records; the
    post-pin targeted classifier reports `pinnedCount=272` and zero remaining
    unpinned candidates for the BLAKE2F prefix. The pinned smoke gate now
    reports `state.count=476`, `blockchain.count=272`, and
    `fixtureCaseCount=801`; the refreshed consolidated drift map reports 612
    remaining candidates: 469 passing state selectors, 90 passing blockchain
...
- [x] `PINNED-V5.4.0-CHAINID-EIP1559-BLOCKCHAIN`: Pin official CHAINID and
  EIP-1559 transaction-validity blockchain replay selectors.
  - Milestone: 3 / Phase A consensus fixtures
  - Dependencies: installed `ethereum/execution-spec-tests` v5.4.0 stable
    fixture root.
  - Acceptance: the unpinned Shanghai `engineNewPayloadV2` selectors from
    `istanbul/eip1344_chainid/test_chainid.json` and
    `london/eip1559_fee_market_change/test_eip1559_tx_validity.json` are
    included in the pinned Phase A blockchain replay table, classify with zero
    remaining unpinned candidates for both prefixes, and pass the pinned
    v5.4.0 smoke gate.
  - Validation: `sbcl --script scripts/classify-blockchain-replay-selectors.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --prefix istanbul/eip1344_chainid --json`;
    `sbcl --script scripts/classify-blockchain-replay-selectors.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --prefix london/eip1559_fee_market_change/test_eip1559_tx_validity.json --json`;
    `sbcl --script scripts/phase-a-drift-map.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --failures-only --json`;
    `sbcl --script scripts/phase-a-smoke-gate.lisp -- --pinned-v5.4.0 --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --json`;
    `sbcl --script tests/run-tests.lisp`.
  - Result (2026-07-01): added four official Shanghai
    `engineNewPayloadV2` replay selectors covering `CHAINID` behavior across
    legacy, access-list, and dynamic-fee transaction envelopes plus valid
    EIP-1559 typed transaction execution. The pre-pin targeted classifiers
    reported 4/4 unpinned candidates passing with zero drift/bug/harness
    records; the post-pin targeted classifiers report `pinnedCount=280` and
    zero remaining unpinned candidates for both prefixes. The pinned smoke
    gate now reports `state.count=476`, `blockchain.count=280`, and
    `fixtureCaseCount=809`; the refreshed consolidated drift map reports 604
    remaining candidates: 469 passing state selectors, 82 passing blockchain
    replay selectors, and 53 Prague/EIP-7702 transaction selectors out of
    scope.

- [x] `PINNED-V5.4.0-CREATE-BOUNDARY-BLOCKCHAIN`: Pin remaining official
  CREATE boundary blockchain replay selectors.
  - Milestone: 3 / Phase A consensus fixtures
  - Dependencies: installed `ethereum/execution-spec-tests` v5.4.0 stable
    fixture root.
  - Acceptance: the remaining unpinned Shanghai `engineNewPayloadV2`
    selectors from `frontier/create/test_create_deposit_oog.json`,
    `frontier/create/test_create_one_byte.json`, and
    `frontier/create/test_create_suicide_store.json` are included in the
    pinned Phase A blockchain replay table, classify with zero remaining
    unpinned candidates for the `frontier/create` prefix, and pass the pinned
    v5.4.0 smoke gate.
  - Validation: `sbcl --script scripts/classify-blockchain-replay-selectors.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --prefix frontier/create --json`;
    `sbcl --script scripts/phase-a-drift-map.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --failures-only --json`;
    `sbcl --script scripts/phase-a-smoke-gate.lisp -- --pinned-v5.4.0 --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --json`;
    `sbcl --script tests/run-tests.lisp`.
  - Result (2026-07-01): added six official Shanghai
    `engineNewPayloadV2` replay selectors covering CREATE and CREATE2 deposit
    out-of-gas, one-byte initcode, and suicide-store creation boundaries. The
    pre-pin targeted classifier reported 6/6 unpinned candidates passing with
    zero drift/bug/harness records; the post-pin targeted classifier reports
    `pinnedCount=286` and zero remaining unpinned candidates for the
    `frontier/create` prefix. The pinned smoke gate now reports
    `state.count=476`, `blockchain.count=286`, and `fixtureCaseCount=815`;
    the refreshed consolidated drift map reports 598 remaining candidates:
...
- [x] `PINNED-V5.4.0-PRECOMPILE-BOUNDARY-BLOCKCHAIN`: Pin official
  precompile-absence and identity-precompile blockchain replay selectors.
  - Milestone: 3 / Phase A consensus fixtures
  - Dependencies: installed `ethereum/execution-spec-tests` v5.4.0 stable
    fixture root.
  - Acceptance: the remaining unpinned Shanghai `engineNewPayloadV2`
    selectors from `frontier/precompiles/test_precompile_absence.json`,
    ...
    are included in the pinned Phase A blockchain replay table, classify with
    zero remaining unpinned candidates for both prefixes, and pass the pinned
    v5.4.0 smoke gate.
  - Validation: `sbcl --script scripts/classify-blockchain-replay-selectors.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --prefix frontier/precompiles --json`;
    `sbcl --script scripts/classify-blockchain-replay-selectors.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --prefix homestead/identity_precompile --json`;
    `sbcl --script scripts/phase-a-drift-map.lisp -- --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --failures-only --json`;
    `sbcl --script scripts/phase-a-smoke-gate.lisp -- --pinned-v5.4.0 --root /Users/sen/workspace/ethereum-lisp/.cache/eest-v5.4.0/root/fixtures --json`;
    `sbcl --script tests/run-tests.lisp`.
  - Result (2026-07-01): added eleven official Shanghai
    `engineNewPayloadV2` replay selectors covering absent-precompile calls
    with empty, 31-byte, and 32-byte calldata, plus identity-precompile return
    buffer modification and overwrite behavior across `CALL`, `CALLCODE`,
    `DELEGATECALL`, and `STATICCALL`. The pre-pin targeted classifiers
    reported 11/11 unpinned candidates passing with zero drift/bug/harness
    records; the post-pin targeted classifiers report `pinnedCount=300` and
    zero remaining unpinned candidates for both prefixes. The pinned smoke gate
    now reports `state.count=476`, `blockchain.count=300`, and
    `fixtureCaseCount=829`; the refreshed consolidated drift map reports 584
    remaining candidates: 469 passing state selectors, 62 passing blockchain
    replay selectors, and 53 Prague/EIP-7702 transaction selectors out of
    scope.
