...
- **Partial:** txpool admission, concrete HTTP/socket serving, production
  ...
  no-serve CLI runs. Restored-database public RPC smoke now also covers log
  and block filter lifecycles by creating filters, reading historical matches
  or initial changes, rejecting a block-filter id on `eth_getFilterLogs`,
  uninstalling filters, and proving removed filter ids return missing-filter
...
- *Latest result:* the runner proof now extends through live
  ...
  the KZG connection contract to seventeen Engine requests. That same runner
  path now also proves a malformed-count
  `engine_getPayloadBodiesByRangeV2` request rejects with the existing
  ...
  requests. It now also proves one `params:null`
  `engine_getPayloadBodiesByRangeV2` request rejects with the existing
  ...
  connection contract to twenty-two Engine requests. It now also proves one
  empty-object `params` request such as `{}` rejects with the current
  invalid-params `-32602` /
  "engine_getPayloadBodiesByRangeV2 params must include start and count"
...
- *Next:* reuse the same engine-only listener coverage to prove the sibling
  non-KZG hidden-method rejection for `engine_getBlobsV3` instead of
...
- *Done:* Engine payload object conversion, version-gated
  ...
  pending subpool. `--txpool.lifetime` now accepts geth-style composite
  ...
  imported split head, the initialized-datadir split `--datadir PATH --http`
  path, the Engine-only `--dev --http=false` path, and the initialized-datadir
  Engine-only `--datadir PATH --http=false` path through ready/log/pid
  artifacts, authenticated Engine RPC, closed public HTTP port behavior,
  stored JWT/database discovery, and shutdown telemetry. Geth/Hive node-key,
  ...
  The focused Engine-only smoke path now also exercises the authenticated
  Engine handshake expected by Hive-style runners: it probes
  `engine_exchangeCapabilities`, `engine_exchangeTransitionConfigurationV1`,
  and a terminal-total-difficulty mismatch through `/engine`, requires V1/V2
  ...
  null committed transaction location fields, and now checks the pending
  sender nonce so reinserted displaced transactions advance
  `eth_getTransactionCount(..., "pending")` at the script process boundary.
  Public txpool
...
- *Done:* fixture root discovery, pinned execution-spec-tests release metadata,
  ...
  The pinned blockchain replay table now also includes six official Shanghai
  `engineNewPayloadV2` selectors from
  `frontier/create/test_create_suicide_during_transaction_create.json`,
  covering CREATE and CREATE2 init-code SELFDESTRUCT variants. The pinned
  smoke gate now executes 735 total fixture cases with 206 blockchain replay
  selectors, and the refreshed v5.4.0 drift map classifies the remaining 678
  candidates as 469 passing state selectors, 156 passing blockchain replay
  ...
  The pinned blockchain replay table now also includes the remaining 30
  official Shanghai `engineNewPayloadV2` EIP-7610 create-collision selectors
  for non-empty code and non-empty nonce target-account collisions across
  CREATE, CREATE2, and tx types 0/1/2. The pinned smoke gate now executes 765
  total fixture cases with 236 blockchain replay selectors, and the refreshed
  v5.4.0 drift map classifies the remaining 648 candidates as 469 passing
  state selectors, 126 passing blockchain replay selectors, and 53
  ...
  fixture-harness-error records.
  The pinned blockchain replay table now also includes four official Shanghai
  `engineNewPayloadV2` selectors covering `CHAINID` across legacy,
  access-list, and dynamic-fee transaction envelopes plus valid EIP-1559
  typed transaction execution. Pre-pin classification found all four unpinned
  selectors passing with zero drift/bug/harness records. The pinned smoke gate
  now executes 809 total fixture cases with 280 blockchain replay selectors,
  and the refreshed v5.4.0 drift map classifies the remaining 604 candidates
  as 469 passing state selectors, 82 passing blockchain replay selectors, and
  53 Prague/EIP-7702 transaction selectors out of scope, with zero
  known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  The pinned blockchain replay table now also includes six official Shanghai
  `engineNewPayloadV2` selectors covering CREATE and CREATE2 deposit
  out-of-gas, one-byte initcode, and suicide-store creation boundaries.
  Pre-pin classification found all six unpinned selectors passing with zero
  drift/bug/harness records. The pinned smoke gate now executes 815 total
  fixture cases with 286 blockchain replay selectors, and the refreshed
  v5.4.0 drift map classifies the remaining 598 candidates as 469 passing
  state selectors, 76 passing blockchain replay selectors, and 53
  Prague/EIP-7702 transaction selectors out of scope, with zero
  known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  The pinned blockchain replay table now also includes three official
  Shanghai `engineNewPayloadV2` selectors covering EIP-145 `SAR`, `SHL`, and
  `SHR` behavior through full blockchain import. Pre-pin classification found
  all three unpinned selectors passing with zero drift/bug/harness records.
  The pinned smoke gate now executes 818 total fixture cases with 289
  blockchain replay selectors, and the refreshed v5.4.0 drift map classifies
  the remaining 595 candidates as 469 passing state selectors, 73 passing
  blockchain replay selectors, and 53 Prague/EIP-7702 transaction selectors
  out of scope, with zero known-implementation-drift,
  implementation-bug-candidate, or fixture-harness-error records.
  The pinned blockchain replay table now also includes eleven official
  Shanghai `engineNewPayloadV2` selectors covering absent-precompile calls
  ...
  `DELEGATECALL`, and `STATICCALL`. Pre-pin classification found all eleven
  unpinned selectors passing with zero drift/bug/harness records. The pinned
  smoke gate now executes 829 total fixture cases with 300 blockchain replay
  selectors, and the refreshed v5.4.0 drift map classifies the remaining 584
  candidates as 469 passing state selectors, 62 passing blockchain replay
  selectors, and 53 Prague/EIP-7702 transaction selectors out of scope, with
  zero known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  The pinned blockchain replay table now also includes 29 official Shanghai
  `engineNewPayloadV2` selectors covering `CALLDATACOPY`, `CALLDATALOAD`, and
  ...
  Pre-pin classification found all 29 unpinned selectors passing with zero
  drift/bug/harness records. The pinned smoke gate now executes 858 total
  fixture cases with 329 blockchain replay selectors, and the refreshed v5.4.0
  drift map classifies the remaining 555 candidates as 469 passing state
  selectors, 33 passing blockchain replay selectors, and 53 Prague/EIP-7702
  transaction selectors out of scope, with zero known-implementation-drift,
  implementation-bug-candidate, or fixture-harness-error records.
  The pinned blockchain replay table now also includes 16 official Shanghai
  `engineNewPayloadV2` selectors covering SWAP1 through SWAP16 stack-underflow
  exceptional halt behavior through full blockchain import. Pre-pin
  classification found all 16 unpinned selectors passing with zero
  drift/bug/harness records. The pinned smoke gate now executes 874 total
  fixture cases with 345 blockchain replay selectors, and the refreshed v5.4.0
  drift map classifies the remaining 539 candidates as 469 passing state
  ...
  transaction selectors out of scope, with zero known-implementation-drift,
  implementation-bug-candidate, or fixture-harness-error records.
  The pinned state-transition table now also includes 48 official
  London/Shanghai frontier identity-precompile selectors covering `CALL` and
  `CALLCODE` value, insufficient-balance, exact-gas, insufficient-gas,
  large-parameter, and returndata-size boundaries. Pre-pin classification
  found all 48 unpinned selectors passing with zero drift/bug/harness records.
  The pinned smoke gate now executes 939 total fixture cases with 524 state
  selectors and 362 blockchain replay selectors, and the refreshed v5.4.0
  drift map classifies the remaining 474 candidates as 421 passing state
  selectors and 53 Prague/EIP-7702 transaction selectors out of scope, with
  zero known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  The pinned state-transition table now also includes 20 official
  London/Shanghai frontier CREATE boundary selectors covering CREATE/CREATE2
  code-deposit out-of-gas, SELFDESTRUCT during initcode, and suicide-store
  creation behavior. Pre-pin classification found all 20 unpinned selectors
  passing with zero drift/bug/harness records. The pinned smoke gate now
  executes 959 total fixture cases with 544 state selectors and 362 blockchain
  replay selectors, and the refreshed v5.4.0 drift map classifies the
  remaining 454 candidates as 401 passing state selectors and 53
  Prague/EIP-7702 transaction selectors out of scope, with zero
  known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  The pinned state-transition table now also includes 16 official
  London/Shanghai frontier value-transfer gas calculation selectors covering
  `CALL`, `CALLCODE`, `DELEGATECALL`, and `STATICCALL` gas-shortage
  boundaries. Pre-pin classification found all 16 unpinned selectors passing
  with zero drift/bug/harness records. The pinned smoke gate now executes 975
  total fixture cases with 560 state selectors and 362 blockchain replay
  selectors, and the refreshed v5.4.0 drift map classifies the remaining 438
  candidates as 385 passing state selectors and 53 Prague/EIP-7702 transaction
  selectors out of scope, with zero known-implementation-drift,
  implementation-bug-candidate, or fixture-harness-error records.
  The pinned state-transition table now also includes 16 official
  ...
  `CALLCODE`, `DELEGATECALL`, and `STATICCALL`. Pre-pin classification found
  all 16 unpinned selectors passing with zero drift/bug/harness records. The
  pinned smoke gate now executes 991 total fixture cases with 576 state
  selectors and 362 blockchain replay selectors, and the refreshed v5.4.0
  drift map classifies the remaining 422 candidates as 369 passing state
  selectors and 53 Prague/EIP-7702 transaction selectors out of scope, with
  zero known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  The pinned state-transition table now also includes 45 official Shanghai
  Paris EIP-7610 CREATE collision selectors covering CREATE/CREATE2 opcode
  collisions and legacy/access-list/dynamic-fee contract-creation transaction
  collisions against non-empty balance, code, and nonce accounts. Pre-pin
  classification found all 45 unpinned selectors passing with zero
  drift/bug/harness records. The pinned smoke gate now executes 1036 total
  fixture cases with 621 state selectors and 362 blockchain replay selectors,
  and the refreshed v5.4.0 drift map classifies the remaining 377 candidates
  ...
  known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  The pinned state-transition table now also includes 32 official
  London/Shanghai Frontier `CALLDATALOAD` and `CALLDATASIZE` selectors
  covering transaction and contract calldata sources, short and word-plus
  calldata loads, and zero/2/16/33/257-byte calldata sizes. Pre-pin targeted
  drift-map classification found all 32 unpinned candidates passing with zero
  ...
  `frontier/opcodes/test_calldatasize.json` prefixes. The pinned smoke gate
  now executes 1100 total fixture cases with 685 state selectors and 362
  blockchain replay selectors, and the refreshed v5.4.0 drift map classifies
  the remaining 313 candidates as 260 passing state selectors and 53
  Prague/EIP-7702 transaction selectors out of scope, with zero
  known-implementation-drift, implementation-bug-candidate, or
  fixture-harness-error records.
  ...
  blockchain replay selectors. The refreshed v5.4.0 drift map reports zero
  state candidates and zero blockchain candidates remaining; the only
  remaining 53 candidates are Prague/EIP-7702 transaction selectors classified
  as `out-of-scope-fork-feature`, with zero known-implementation-drift,
  implementation-bug-candidate, or fixture-harness-error records and
  `phaseAMaterializableClear=true`.
