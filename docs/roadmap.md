...
code/storage-preserving updates. Storage-root fixtures now lock zero-value
writes to absent storage slots as no-ops across branch-shaped and
extension-shaped secure storage tries. The trie harness now covers secure-key
branch, extension, delete-collapse, delete-to-empty, and missing-delete no-op
replay in both seed vectors and selected EEST-style secureTrie samples,
including no-op deletion over branch and path-compressed extension roots.
Persistence
