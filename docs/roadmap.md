...
- *Done:* Engine payload object conversion, version-gated
  ...
  Engine readiness batch, plus public `eth_chainId`, `web3_clientVersion`,
  `net_version`, `net_listening`, and `eth_syncing` requests plus a public
  JSON-RPC readiness batch through its ready-file endpoints, with shutdown
  telemetry accounting for the externally served readiness connections. The
  same script subprocess
