...
| Dial ratio (outbound share) | 3 normally; an active pivot import counts SNAP-capable outbound sessions toward this target while the absolute 50-peer limit remains authoritative (`+devnet-dial-ratio+`, `devnet/dial-schedule.lisp`) | 3 (`defaultDialRatio`, `p2p/server.go:50`; applied `:557`) | no equivalent constant |
