...
- `src/networking/snap-sync/backend.lisp` and `client.lisp` provide the verified
  ...
  Exact accepted revision `7ee257f2ce62564029888eb93c40cbb650144062`
  adds a second
  ...
  set. Independent review rejected a draft that called the requester directly;
  the accepted test instead runs the production session pump, and a second RED
  control fails when its drain action no longer invokes that requester. The
  one-file delta passes the focused selector, the three-test session-pump unit
  ...
  Exact accepted revision `7449e76611857813563a1ec8a7248095e8c5e772`
  covers pinned `TestSnapTrieNodes` known-account path cardinality. A
  deterministic three-account fixture sends 65 singleton compact paths, one for
  every secure-key prefix length from 1 through 65, through the production
  eth+snap session pump and persistent backend under a 5,000-byte budget. The
  response preserves request ID 83 and all positions: two available encoded
  nodes followed by 63 zero-length placeholders. Omitting unavailable nodes in
  the backend failed the exact 65-node cardinality assertion. The focused test,
  twelve-test session-pump family, 128-test SNAP unit family, and full
  1,374-test cold-unit layer passed with three optional skips after final
  cleanup. Independent review approved exact makeSnapPath/terminator parity,
  fixture shape, assertions, and production boundary without blockers. This
  remains local regression evidence rather than a successor Hive pass; exact
  evidence is in
  ...
  Exact accepted revision `5fda6c277655c61a201d9865fe3a60e2852e6571`
  covers the adjacent pinned `TestSnapGetByteCodes` duplicate empty-code-hash
  case. A request carrying the canonical empty code hash three times through the
  production eth+snap session pump and persistent backend returns three
  zero-length items under the matching request ID; requested hashes are not
  deduplicated. A duplicate-removal mutation failed the exact cardinality
  assertion. The focused test, fourteen-test session-pump family, 130-test SNAP
  unit family, and full 1,376-test cold-unit layer passed with three optional
  skips. Independent review approved the one-file test delta without required
  fixes. This remains local regression evidence rather than a successor Hive
