...
non-KZG hidden `engine_getBlobsV2` listener proof on top of the existing
...
sibling non-KZG hidden-method rejection for `engine_getBlobsV3` instead of
...
contract for `engine_getBlobsV3`. The best bounded follow-up is to send one
...
for `engine_getBlobsV1` and `engine_getBlobsV2`, and record the result before
widening into unrelated Phase B runner work.
