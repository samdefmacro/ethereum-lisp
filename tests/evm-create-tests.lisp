      ...
      (let* ((state (make-state-db))
        ...
        (state-db-set-account state creator
                              (make-state-account :nonce max-nonce
                                                  :balance balance))
        ;; A creator at the maximum nonce cannot create: CREATE/CREATE2 push 0
        ;; and continue (no hard error), leaving nonce and balance untouched.
        (let ((result (execute-bytecode code :context context)))
...
(deftest evm-create-collision-rejects-storage-not-balance-accounts
  ;; EIP-7610 / EIP-684: a create target collides when it has a nonzero nonce,
  ;; non-empty code, or non-empty storage. A pre-funded (balance-only) address
  ;; does NOT collide, so the creation succeeds and keeps its balance.
  (let ((creator (address-from-hex
                  "0x00000000000000000000000000000000000000aa"))
        (create-code
          (concat-bytes
    ...
    (labels ((expected-address ()
               (make-address
                (subseq (keccak-256
                         (rlp-encode
                          (make-rlp-list (address-bytes creator) 0)))
                        12 32))))
      ;; Balance-only target: creation succeeds.
      (let* ((state (make-state-db))
             (context (make-evm-context :state state :address creator))
        ...
        (let ((result (execute-bytecode create-code
          ...
          (is (= 1 (state-account-nonce
                    (state-db-get-account state creator))))
          (is (plusp (length (state-db-get-code state address))))
          (is (= 1 (state-account-balance
                    (state-db-get-account state address))))))
      ;; Storage-only target: creation collides and fails.
      (let* ((state (make-state-db))
             (context (make-evm-context :state state :address creator))
             (address (expected-address)))
        (state-db-set-account state creator (make-state-account :balance 10))
        (state-db-set-storage state address slot 2)
        (let ((result (execute-bytecode create-code
                                        :context context
                                        :gas-limit 100000)))
          (is (= 0 (first (evm-result-stack result))))
          (is (= 1 (state-account-nonce
                    (state-db-get-account state creator))))
          (is (= 2 (state-db-get-storage state address slot))))))))
