...
(deftest eth-peer-run-session-ignores-a-storage-root-as-a-snap-account-root
  (:layer :unit :module :p2p)
  ;; Pinned geth 101035a1 TestSnapGetAccountRange lines 237-247 requests an
  ;; existing account's storage root as a state root and requires an empty
  ;; response rather than exposing the current account trie under the wrong root.
  (let* ((state (make-state-db))
         (database (make-memory-key-value-database))
         (peer (ethereum-lisp.eth-sync::%make-eth-peer))
         (address
           (address-from-hex
            "0x0000000000000000000000000000000000005678"))
         (slot (make-hash32 (make-byte-vector 32 :initial-element #x42)))
         (read-symbol 'ethereum-lisp.eth-sync:eth-peer-read-once)
         (send-symbol 'ethereum-lisp.eth-sync:eth-peer-send-snap)
         (real-read (fdefinition read-symbol))
         (real-send (fdefinition send-symbol))
    ...
    (let* ((storage-root
             (hash32-bytes (state-db-get-storage-root state address)))
           (backend
             (ethereum-lisp.snap-sync:make-persistent-snap-state-backend
              database state
              :state-provider
           ...
           (request
             (ethereum-lisp.snap:make-snap-get-account-range
              95 storage-root (make-byte-vector 32)
              (make-byte-vector 32 :initial-element #xff) 4000))
           (payload
             (ethereum-lisp.snap:encode-snap-message
              ethereum-lisp.snap:+snap-message-get-account-range+ request)))
      (is (not (bytes= storage-root
                       (hash32-bytes (state-db-root state)))))
      (setf (ethereum-lisp.eth-sync::eth-peer-snap-offset peer) 100
            (ethereum-lisp.eth-sync::eth-peer-snap-backend peer) backend)
      (unwind-protect
        ...
        (setf (fdefinition read-symbol) real-read
              (fdefinition send-symbol) real-send))
      (is (= 1 (length provider-roots)))
      (is (bytes= storage-root (first provider-roots)))
      (is sent)
      (is (= ethereum-lisp.snap:+snap-message-account-range+ (first sent)))
      (let ((response (second sent)))
        (is (= 95 (ethereum-lisp.snap:snap-account-range-id response)))
        (is (null
             (ethereum-lisp.snap:snap-account-range-accounts response)))
        (is (null
             (ethereum-lisp.snap:snap-account-range-proof response)))))))

(deftest eth-peer-run-session-preserves-inverted-snap-account-range-boundaries
  (:layer :unit :module :p2p)
  ;; Pinned geth 101035a1 TestSnapGetAccountRange lines 251-287 distinguishes
  ;; an origin beyond all keys from an origin at a real key.  Inverted limits do
  ;; not reject that available origin: the server returns it once and stops.
  (let* ((state (make-state-db))
         (database (make-memory-key-value-database))
         (peer (ethereum-lisp.eth-sync::%make-eth-peer))
         (addresses
    ...
    (loop for address in addresses
          ...
          do (state-db-set-account
              state address (make-state-account :balance balance)))
    (let* ((root (hash32-bytes (state-db-root state)))
           (entries (state-db-account-range state))
           (first-key (state-account-range-entry-proof-key (first entries)))
           (first-minus-one
             (ethereum-lisp.crypto::integer-to-fixed-bytes
              (1- (bytes-to-integer first-key)) 32))
           (backend
           ...
           (requests
             (list
              (ethereum-lisp.snap:make-snap-get-account-range
               96 root maximum zero 4000)
              (ethereum-lisp.snap:make-snap-get-account-range
               97 root first-key first-minus-one 4000)
              (ethereum-lisp.snap:make-snap-get-account-range
               98 root first-key zero 4000))))
      (is (= 3 (length entries)))
      (is (plusp (bytes-to-integer first-key)))
      (setf payloads
            (mapcar
             (lambda (request)
               (ethereum-lisp.snap:encode-snap-message
      ...
      (is (= 3 (length sent)))
      (loop for response-entry in sent
            for request-id in '(96 97 98)
            for expected in (list nil (list first-key) (list first-key))
               ...
               (let* ((response (second response-entry))
                      (accounts
                        (ethereum-lisp.snap:snap-account-range-accounts
                         response))
                      (actual
                        (mapcar #'ethereum-lisp.snap:snap-account-data-hash
                                accounts)))
                 (is (= request-id
...
(deftest eth-peer-run-session-serves-first-snap-account-over-soft-limit
  (:layer :unit :module :p2p)
  ;; Pinned geth 101035a1 TestSnapGetAccountRange lines 115-135 requires the
  ;; first available account when the response-byte target is one or zero.
  (let* ((state (make-state-db))
         (database (make-memory-key-value-database))
         (peer (ethereum-lisp.eth-sync::%make-eth-peer))
         (addresses
    ...
    (let* ((root (hash32-bytes (state-db-root state)))
           ...
           (requests
             (list
              (ethereum-lisp.snap:make-snap-get-account-range
               99 root (make-byte-vector 32)
               (make-byte-vector 32 :initial-element #xff) 1)
              (ethereum-lisp.snap:make-snap-get-account-range
               100 root (make-byte-vector 32)
               (make-byte-vector 32 :initial-element #xff) 0))))
      (is (= 3 (length entries)))
      (setf payloads
            (mapcar
             (lambda (request)
...
(deftest eth-peer-run-session-stops-snap-account-range-after-soft-limit-crossing
  (:layer :unit :module :p2p)
  ;; Pinned geth 101035a1 ServiceGetAccountRangeQuery handlers.go lines 82-102
  ;; appends an account before stopping only when cumulative size exceeds Bytes.
  ;; TestSnapGetAccountRange lines 84-113 exercises decreasing response budgets.
  (let* ((state (make-state-db))
         (database (make-memory-key-value-database))
         (peer (ethereum-lisp.eth-sync::%make-eth-peer))
         (addresses
    ...
    (let* ((root (hash32-bytes (state-db-root state)))
           (backend
             (ethereum-lisp.snap-sync:make-persistent-snap-state-backend
              database state))
           (baseline
             (ethereum-lisp.snap-sync::snap-sync-account-response
              database state
              (ethereum-lisp.snap:make-snap-get-account-range
               106 root zero maximum 100000)))
           (baseline-accounts
             (ethereum-lisp.snap:snap-account-range-accounts baseline))
           (keys
             (mapcar #'ethereum-lisp.snap:snap-account-data-hash
                     baseline-accounts))
           (sizes
             (mapcar
              ...
              baseline-accounts))
           (first-budget (first sizes))
           (second-budget (+ first-budget (second sizes)))
           (requests
             (list
              (ethereum-lisp.snap:make-snap-get-account-range
               107 root zero maximum first-budget)
              (ethereum-lisp.snap:make-snap-get-account-range
               108 root zero maximum second-budget))))
      (is (= 4 (length baseline-accounts)))
      (is (every #'plusp sizes))
      (setf payloads
            (mapcar
             (lambda (request)
               (ethereum-lisp.snap:encode-snap-message
      ...
      (loop for response-entry in sent
            ...
            do (is (= ethereum-lisp.snap:+snap-message-account-range+
               ...
               (let* ((response (second response-entry))
                 ...
                 (is (= request-id
                        (ethereum-lisp.snap:snap-account-range-id response)))
                 (is (= count (length actual)))
                 (loop for wanted in (subseq keys 0 count)
                       for received in actual
                       do (is (bytes= wanted received))))))))

(deftest eth-peer-run-session-preserves-snap-account-range-boundaries
  (:layer :unit :module :p2p)
  ;; Pinned geth 101035a1 TestSnapGetAccountRange lines 138-197 requires the
  ;; first available key at or beyond the origin and the first key at or beyond
  ;; the inclusive limit, even when that key lies outside the numeric interval.
  (let* ((state (make-state-db))
         (database (make-memory-key-value-database))
         (peer (ethereum-lisp.eth-sync::%make-eth-peer))
         (addresses
    ...
    (let* ((root (hash32-bytes (state-db-root state)))
           (entries (state-db-account-range state))
           (keys (mapcar #'state-account-range-entry-proof-key entries))
           (first-key (first keys))
           (second-key (second keys))
           (first-integer (bytes-to-integer first-key))
           (first-minus-500
             (ethereum-lisp.crypto::integer-to-fixed-bytes
              (- first-integer 500) 32))
           (first-minus-450
             (ethereum-lisp.crypto::integer-to-fixed-bytes
              (- first-integer 450) 32))
           (first-plus-one
             (ethereum-lisp.crypto::integer-to-fixed-bytes
              (1+ first-integer) 32))
           (backend
           ...
           (requests
             (list
              (ethereum-lisp.snap:make-snap-get-account-range
               101 root first-minus-500 first-plus-one 4000)
              (ethereum-lisp.snap:make-snap-get-account-range
               102 root first-minus-500 first-minus-450 4000)
              (ethereum-lisp.snap:make-snap-get-account-range
               103 root zero zero 4000)
              (ethereum-lisp.snap:make-snap-get-account-range
               104 root first-key maximum 4000)
              (ethereum-lisp.snap:make-snap-get-account-range
               105 root first-plus-one maximum 4000))))
      (is (= 3 (length keys)))
      (is (> first-integer 500))
      (is (ethereum-lisp.validation:byte-vector-lexicographic<
      ...
      (is (= 5 (length sent)))
      (loop for response-entry in sent
            for request-id in '(101 102 103 104 105)
            for expected in (list (list first-key second-key)
                                  (list first-key)
                                  (list first-key)
                                  keys
                                  (rest keys))
