  ...
  (let ((script (namestring (truename "scripts/ethereum-lisp.lisp"))))
    (labels ((run-script (&rest args)
      ...
      (multiple-value-bind (stdout stderr status)
        ...
        (is (search "ethereum-lisp db --help" stdout)))
      (multiple-value-bind (stdout stderr status)
          (run-script "--help")
        (is (= 0 status))
        (is (string= "" stderr))
      ...
      (multiple-value-bind (stdout stderr status)
        ...
        (is (= 0 status))
        (is (string= "" stderr))
        (is (search "Usage: ethereum-lisp db" stdout))
        (is (search "verify" stdout))
        (is (search "backup" stdout))
        (is (search "repair" stdout))
        (is (search "rebuild" stdout)))
      (multiple-value-bind (stdout stderr status)
          (run-script "version")
        (is (= 0 status))
        (is (string= "" stderr))
