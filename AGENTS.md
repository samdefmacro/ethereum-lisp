...
Use `cl-workbench` and the documented Docker targets for every application
build, eval, test, documentation run, generated-code step, and deployable
process. Never invoke an application interpreter/compiler on the host, and
never fall back when Docker or a declared Workbench capability is unavailable.
