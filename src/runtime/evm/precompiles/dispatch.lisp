  ...
  (let ((number (address-to-word address)))
    (when (active-precompile-address-p address rules)
      (case number
        ...
        (11 +bls12381-g1-add-gas+)
        (12 (bls12381-g1-msm-gas input))
        (13 +bls12381-g2-add-gas+)
        (14 (bls12381-g2-msm-gas input))
        (15 (bls12381-pairing-gas input))
        (16 +bls12381-map-fp-to-g1-gas+)
        (17 +bls12381-map-fp2-to-g2-gas+)
        (256 +p256verify-gas+)
        (otherwise nil)))))
    ...
    (case (address-to-word address)
      ...
      (10 (if (active-precompile-address-number-p 10 rules)
              (multiple-value-bind (output gas)
                ...
                (values output gas t))
              (values nil 0 nil)))
      (11 (if (active-precompile-address-number-p 11 rules)
              (multiple-value-bind (output gas)
                  (run-bls12381-g1-add-precompile input)
                (values output gas t))
              (values nil 0 nil)))
      (12 (if (active-precompile-address-number-p 12 rules)
              (multiple-value-bind (output gas)
                  (run-bls12381-g1-msm-precompile input)
                (values output gas t))
              (values nil 0 nil)))
      (13 (if (active-precompile-address-number-p 13 rules)
              (multiple-value-bind (output gas)
                  (run-bls12381-g2-add-precompile input)
                (values output gas t))
              (values nil 0 nil)))
      (14 (if (active-precompile-address-number-p 14 rules)
              (multiple-value-bind (output gas)
                  (run-bls12381-g2-msm-precompile input)
                (values output gas t))
              (values nil 0 nil)))
      (15 (if (active-precompile-address-number-p 15 rules)
              (multiple-value-bind (output gas)
                  (run-bls12381-pairing-check-precompile input)
                (values output gas t))
              (values nil 0 nil)))
      (16 (if (active-precompile-address-number-p 16 rules)
              (multiple-value-bind (output gas)
                  (run-bls12381-map-fp-to-g1-precompile input)
                (values output gas t))
              (values nil 0 nil)))
      (17 (if (active-precompile-address-number-p 17 rules)
              (multiple-value-bind (output gas)
                  (run-bls12381-map-fp2-to-g2-precompile input)
                (values output gas t))
              (values nil 0 nil)))
