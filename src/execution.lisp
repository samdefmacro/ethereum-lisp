...
(defun charge-sender-upfront (state sender tx
  ...
  (let* ((sender-account (execution-account-or-empty state sender))
    ...
    (unless (uint256-p value)
      (error 'transaction-validation-error :message "Transaction value exceeds uint256"))
    (unless (transaction-nonce-uint64-p nonce)
      (error 'transaction-validation-error :message "Transaction nonce exceeds uint64"))
    (unless (transaction-gas-limit-uint64-p gas-limit)
      (error 'transaction-validation-error :message "Transaction gas limit exceeds uint64"))
    ...
    (let* ((execution-gas-cost (* gas-limit gas-price))
